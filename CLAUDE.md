...
Known state: `npm test` currently fails — jsdom has no `window.matchMedia` and the section components call it inside their effects; the smoke test's "empty `<main>`" assertion also predates the page build-out.
